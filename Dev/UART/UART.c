...
static  void  EnableUART1Tx(void);          //使能串口发送，在WriteUart中调用，即每次发送数据之后需要调用
                                            //打开TXE的中断开关
...
static  void  ConfigUART(void)
{
  ...
  //开启RCC时钟
  if(USER_DEFINE_USART == USART1 || USER_DEFINE_USART == USART6)    //如果串口为USART1或USART6
  {                                                                 
    RCC_APB2PeriphClockCmd(USER_DEFINE_USART_CLK, ENABLE);          //开启USART1或USART6的时钟   
  }                                                             
  else                                                          
  {                                                             
    RCC_APB1PeriphClockCmd(USER_DEFINE_USART_CLK, ENABLE);          //开启其他UART的时钟
  }
    
  RCC_AHB1PeriphClockCmd(USER_DEFINE_USART_TX_GPIO_CLK, ENABLE);    //开启USART的GPIO时钟
  if(USER_DEFINE_USART_TX_GPIO_CLK != USER_DEFINE_USART_RX_GPIO_CLK)//如果USART.TX和USART.RX的GPIO端口不同
  {
    RCC_AHB1PeriphClockCmd(USER_DEFINE_USART_RX_GPIO_CLK, ENABLE);  //开启USART.RX的时钟
  }
  
  //配置USART.Tx的GPIO 
  GPIO_InitStructure.GPIO_Pin = USER_DEFINE_USART_TX_GPIO_PIN;//设置USART.TX的引脚
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;                //设置USART.TX的模式
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;           //设置USART.TX的I/O口速率
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;              //设置USART.TX的输出类型为推挽输出
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;                //设置USART.TX为上拉模式
  GPIO_Init(USER_DEFINE_USART_TX_GPIO_PORT, &GPIO_InitStructure);//根据参数初始化USART.RX的GPIO端口
  
  //配置USART.Rx的GPIO
  GPIO_InitStructure.GPIO_Pin = USER_DEFINE_USART_RX_GPIO_PIN;//设置USART.RX的引脚
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;                //设置USART.RX的模式
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;           //设置USART.RX的I/O口速率
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;              //设置USART.RX输出类型为推挽输出
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;                //设置USART.RX为上拉模式
  GPIO_Init(USER_DEFINE_USART_RX_GPIO_PORT, &GPIO_InitStructure);//根据参数初始化USART.RX的GPIO端口
  
  //将引脚PA10连接到USART_RX
  GPIO_PinAFConfig(USER_DEFINE_USART_RX_GPIO_PORT, USER_DEFINE_USART_RX_SOURCE, USER_DEFINE_USART_RX_AF);
  //将引脚PA9连接到USART_TX
  GPIO_PinAFConfig(USER_DEFINE_USART_TX_GPIO_PORT, USER_DEFINE_USART_TX_SOURCE, USER_DEFINE_USART_TX_AF);
  ...
  //USART常规参数设置
  USART_Init(USER_DEFINE_USART, &USART_InitStructure);                          //按照以上参数配置USART1

  //USART NVIC配置
  NVIC_InitStructure.NVIC_IRQChannel                   = USER_DEFINE_USART_IRQ; //开启USART1的中断
  ...
  USART_Cmd(USER_DEFINE_USART, ENABLE);                                      //使能UART1
...
{  
  USART_SendData(USER_DEFINE_USART, (u8)ch);//发送一个字节数据到串口
...
u8  WriteUART(u8 *pBuf, u8 len)
{
  ...
  {                                                                                                     
    ...
    {                                                             
      EnableUART1Tx();                                            //使能串口发送，打开TXE的中断开关
    }                                                             
  }                                                               
