...
static  void  Proc2msTask(void)
{
  ...
  {
    LEDFlicker(250);                //以下部分每2ms执行一次
    
    while(ReadUART(&recData, 1))    //当成功读到串口中长度为1的数据时
    {                               
      WriteUART(&recData, 1);       //将接收到的长度为1的数据写入串口
    }
...
int main(void)
{ 
  InitSoftware();   //初始化软件相关的函数
  InitHardware();   //初始化硬件相关的函数
